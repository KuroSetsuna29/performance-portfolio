...
LATEST_VERSION=$(wget -qO- https://api.github.com/repos/buchen/portfolio/releases/latest | jq -r '.assets | map(select(.browser_download_url | test("-linux.gtk.x86_64.tar.gz$"))) | first | .browser_download_url')
